...
def loaddata():
    ...
    #C:\Users\jonme\Documents\GitHub\FBI\Streamlit\Data\MN2022 NIBRS Report data.csv
    #PostGRE Database query output stored as CSV
    #Edit the ConnectTODB.py to change the query parameters
    st.session_state.df = pd.read_csv(filename)

#define the sidebar to select and load the year of choice.  For MN we have 2018-2022.  Init with 2022 and pass to loaddata()
st.sidebar.selectbox(label="Choose Year", options=(2022,2021,2020,2019,2018),index=0, key="year", on_change=loaddata())
st.sidebar.write("""
                 The data presented here is a subset of the MN NIBRS dataset.  The original data was download as a PostGRE SQL database and queried for various statistics, and offender demographics.  Data for Minnesota is available from 2018-2022.
                 """)

# Loading the offense dictionary from the text file
# ---->Update needed<----
#FUTURE - change this to a db query using the nibrs_offense_type table
offenses = {}
...
def main():
    ...
    st.write('''
            ...
            ''')  
    #fig2 = px.pie(data_frame=filter[["incident","race"]], names='race', values='incident' )
